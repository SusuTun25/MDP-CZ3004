...
from entities.settings import colors, settings
from entities.settings.direction import Direction
from entities.robot.movement.command import Command
from entities.robot.movement.straight_command import StraightCommand
from entities.robot.movement.turn_command import TurnCommand
from entities.map.position import RobotPosition
from entities.robot.algo.algo import Algo
...
class Robot:
    def __init__(self, grid):
        ...
        self.brain = Algo(self, grid)

        self.__image = pygame.transform.scale(pygame.image.load("entities/image/pug.gif"),
    ...
    def draw_historic_path(self, screen):
        for dot in self.path_hist:
            pygame.draw.circle(screen, colors.BLACK, dot, 1)
